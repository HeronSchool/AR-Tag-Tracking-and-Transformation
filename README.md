...
The project focuses on detecting ArUco markers and get the position and rotation data. There are 2 modes of detection. The first one is base, which is used for base coordinates by detecting 3 markers. The second one is dynamic, which is used for deteing a movement of a marker. All the data of markers is stored in csv files and will be used to calculate the transformation of one coordinate system to another.
